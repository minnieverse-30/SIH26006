...
def calculate_risk(
    ...
    if port_delay_days < 0:
        raise ValueError("Port delay days cannot be negative.")
    ...
    # --------------------------------
    if vessel_availability.upper() != "AVAILABLE":
        risk_score += 20
        risk_factors.append({
            "factor": "Vessel Availability",
            "level": "HIGH",
            "reason": "Selected vessel is not currently available."
        })

    # --------------------------------
    # 4. CONTRACT FLEXIBILITY RISK
    # --------------------------------
    if contract_flexibility.upper() == "FIXED":
        risk_score += 10
        risk_factors.append({
            "factor": "Contract Flexibility",
            "level": "MEDIUM",
            "reason": "Fixed contracts provide less flexibility if market conditions change."
        })
