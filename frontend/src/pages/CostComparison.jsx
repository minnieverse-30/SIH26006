...
function CostComparison() {
  ...
  const fetchCost = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || "Failed to load cost data");
      }

      setCostData(data.data);
    } catch (err) {
      console.error("COST ERROR:", err);
      setError(err.message || "Unable to load cost analysis.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalCost = Number(costData.total_expected_cost || 0);
  const freightCost = Number(breakdown.freight_cost || 0);
  const fuelCost = Number(breakdown.fuel_cost || 0);
  const portCost = Number(breakdown.port_cost || 0);
  const idleCost = Number(breakdown.idle_cost || 0);
  const riskCost = Number(breakdown.risk_cost || 0);

  const extraCosts = fuelCost + portCost + idleCost + riskCost;
  ...
  return (
    <div className="min-h-screen p-8 bg-slate-100">
      ...
      <div className="p-5 mb-6 bg-white border shadow-sm rounded-xl border-slate-200">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="px-4 py-2 rounded-lg bg-slate-50">
              <p className="text-xs text-slate-500">Forecast Rate</p>
              <p className="text-sm font-semibold text-slate-800">
                {formatCurrency(costData.forecast_freight_rate)}/MT
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 gap-5 mb-6 md:grid-cols-3">
        ...
        <div className="p-5 bg-white border shadow-sm rounded-xl border-slate-200">
          <p className="text-sm text-slate-500">Expected Cost / MT</p>

          <h2 className="mt-2 text-2xl font-bold text-slate-800">
            {formatCurrency(costData.expected_cost_per_tonne)}
          </h2>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Current expected cost is calculated using the forecast freight
                rate and configured cost components.
              </p>
            </div>
          </div>
          ...
          <table className="w-full min-w-[800px] text-left">
            <thead className="bg-slate-50">
              <tr className="text-xs tracking-wide uppercase text-slate-500">
                ...
                <th className="px-6 py-4">Per MT</th>
            ...
            <tbody>
              <tr className="border-t border-slate-100">
                ...
                <td className="px-6 py-5 text-sm text-slate-700">
                  {formatCurrency(costData.forecast_freight_rate)}
                </td>
              ...
              <tr className="border-t border-slate-100">
                ...
                <td className="px-6 py-5 text-sm text-slate-700">
                  {formatCurrency(fuelCost / cargoQuantity)}
                </td>
              ...
              <tr className="border-t border-slate-100">
                ...
                <td className="px-6 py-5 text-sm text-slate-700">
                  {formatCurrency(portCost / cargoQuantity)}
                </td>
              ...
              <tr className="border-t border-slate-100">
                ...
                <td className="px-6 py-5 text-sm text-slate-700">
                  {formatCurrency(idleCost / cargoQuantity)}
                </td>
              ...
              <tr className="border-t border-slate-100">
                ...
                <td className="px-6 py-5 text-sm text-slate-700">
                  {formatCurrency(riskCost / cargoQuantity)}
                </td>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="max-w-4xl mt-1 text-sm leading-6 text-slate-500">
              The current estimate is primarily driven by the forecast freight
              rate. Additional fuel, port, idle and risk costs can be supplied
              by the backend as more operational data becomes available.
            </p>

            {extraCosts === 0 && (
              <p className="mt-2 text-xs font-medium text-amber-600">
                Note: Fuel, port, idle and risk inputs are currently configured
                as zero for this analysis.
              </p>
            )}
          </div>
        </div>
